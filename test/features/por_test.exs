defmodule Vor.Features.PORTest do
  ...
  POR is another reduction that silently drops states; the cautionary tale is the
  old symmetry reduction (since removed — see KNOWN_ISSUES §2), which produced a
  wrong number on a degenerate space and nobody noticed. So the bar is: **POR
  must never change a verdict or a relevance label, and must never hide a
  counterexample.** These tests compare POR-on against POR-off on every relevant
  shape.
  """
  ...
  test "POR preserves the Raft per-term PROOF (verdict + substantive relevance)" do
    {verdict, _n_off, _n_on} =
      assert_por_equivalent(@raft,
        max_depth: 40, max_queue: 2, integer_bound: 2, max_states: 2_000_000
      )
  ...
  test "POR preserves the Raft counterexample (global invariant stays VIOLATED)" do
    {verdict, _, _} =
      assert_por_equivalent(@raft_global,
        max_depth: 40, max_queue: 2, integer_bound: 2, max_states: 2_000_000, allow_vacuous: true
      )
  ...
  test "POR preserves vacuity: with timers off, Raft stays vacuous under POR" do
    {verdict, r_off, _} = categorize(Explorer.check_file(@raft, max_depth: 40, max_queue: 2, integer_bound: 2, allow_vacuous: true, fire_timers: false, por: false))
    {verdict2, r_on, _} = categorize(Explorer.check_file(@raft, max_depth: 40, max_queue: 2, integer_bound: 2, allow_vacuous: true, fire_timers: false, por: true))
