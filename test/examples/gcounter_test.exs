defmodule Vor.Examples.GCounterTest do
  ...
  defp start_gcounter(node_id, sync_interval) do
    ...
    {:ok, pid} = :gen_statem.start_link(result.module, [node_id: node_id, sync_interval_ms: sync_interval], [])
    pid
  end
  ...
  test "single node starts with zero" do
    ...
    assert {:total, %{value: 0}} = :gen_statem.call(pid, {:value, %{}})
    :gen_statem.stop(pid)
  end

  test "single node increments correctly" do
    pid = start_gcounter(:test, 10_000)
    :gen_statem.call(pid, {:increment, %{}})
    :gen_statem.call(pid, {:increment, %{}})
    :gen_statem.call(pid, {:increment, %{}})
    assert {:total, %{value: 3}} = :gen_statem.call(pid, {:value, %{}})
    :gen_statem.stop(pid)
  end
  ...
  test "merge with remote state takes max" do
    ...
    :gen_statem.cast(pid, {:sync, %{remote_counts: %{node_b: 5}}})
    Process.sleep(50)

    assert {:total, %{value: 7}} = :gen_statem.call(pid, {:value, %{}})
    :gen_statem.stop(pid)
  end
  ...
  test "merge is idempotent" do
    ...
    remote = %{node_b: 3}
    :gen_statem.cast(pid, {:sync, %{remote_counts: remote}})
    Process.sleep(50)
    :gen_statem.cast(pid, {:sync, %{remote_counts: remote}})
    Process.sleep(50)
    :gen_statem.cast(pid, {:sync, %{remote_counts: remote}})
    ...
    assert {:total, %{value: 4}} = :gen_statem.call(pid, {:value, %{}})
    :gen_statem.stop(pid)
  end

  test "merge takes max per key" do
    pid = start_gcounter(:node_a, 10_000)
    for _ <- 1..5, do: :gen_statem.call(pid, {:increment, %{}})

    # Remote claims node_a=3 (stale) and node_b=10
    :gen_statem.cast(pid, {:sync, %{remote_counts: %{node_a: 3, node_b: 10}}})
    Process.sleep(50)

    # node_a stays at 5 (max 5,3), node_b=10
    assert {:total, %{value: 15}} = :gen_statem.call(pid, {:value, %{}})
    :gen_statem.stop(pid)
  end
  ...
  test "three-node cluster converges after gossip" do
    ...
    for _ <- 1..3, do: :gen_statem.call(n1, {:increment, %{}})
    for _ <- 1..2, do: :gen_statem.call(n2, {:increment, %{}})
    for _ <- 1..5, do: :gen_statem.call(n3, {:increment, %{}})

    # Wait for gossip rounds
    Process.sleep(1000)

    # All nodes should report 3 + 2 + 5 = 10
    {:total, %{value: t1}} = :gen_statem.call(n1, {:value, %{}})
    {:total, %{value: t2}} = :gen_statem.call(n2, {:value, %{}})
    {:total, %{value: t3}} = :gen_statem.call(n3, {:value, %{}})
  ...
  test "counter never decreases" do
    ...
    for _ <- 1..5, do: :gen_statem.call(n1, {:increment, %{}})
    Process.sleep(500)

    {:total, %{value: v1}} = :gen_statem.call(n1, {:value, %{}})
    assert v1 >= 5

    for _ <- 1..3, do: :gen_statem.call(n1, {:increment, %{}})
    Process.sleep(500)

    {:total, %{value: v2}} = :gen_statem.call(n1, {:value, %{}})
    assert v2 >= v1
